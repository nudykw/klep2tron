...
use crate::{ActorSocket, ViewportSettings};
        ...
        if let Some(effect) = &socket.definition.effect {
            ...
            match effect.effect_type {
                EffectType::Plasma => {
                    ...
                    let pulse = (t * effect.motion.speed).sin() * 0.1 + 1.0;
                    let base_radius = 0.2 * effect.visuals.scale * pulse;
                    
                    // Use HSLA for better color shifting
                    let mut hsla = Hsla::from(effect.visuals.color_start);
                ...
                EffectType::Smoke => {
                    ...
                    let speed = effect.motion.speed * 0.3;
                    for i in 0..6 {
                        ...
                        let p = pos + rot * (Vec3::Y * offset * 1.5 + Vec3::new(drift_x, 0.0, drift_z)) * effect.visuals.scale;
                        let r = 0.1 * effect.visuals.scale * (1.0 + offset * 5.0);
                        
                        let mut hsla = Hsla::from(effect.visuals.color_start);
                        hsla.alpha = (1.0 - offset).powf(1.8) * 0.6 * effect.emission.rate;
