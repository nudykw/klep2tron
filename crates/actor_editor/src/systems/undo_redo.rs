...
pub struct AddSocketCommand {
    pub entity: Entity,
    pub definition: crate::SocketDefinition,
}
...
impl Command for AddSocketCommand {
    ...
    fn execute(&self, world: &mut World) {
        if let Some(mut entity) = world.get_entity_mut(self.entity) {
            ...
            entity.insert(crate::ActorSocket {
    ...
    fn undo(&self, world: &mut World) {
        if let Some(mut entity) = world.get_entity_mut(self.entity) {
            ...
            entity.remove::<crate::ActorSocket>();
        }
    }
}
...
fn clear_selected_triangles(world: &mut World, entity: Entity) {
    if let Some(mut sel) = world.get_mut::<crate::SelectedTriangles>(entity) {
        sel.indices.clear();
    }
}
