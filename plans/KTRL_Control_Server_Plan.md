...
> **Статус:** Фазы 1–3 ✅ (2026-09-26); MCP-обёртка не планируется (в pi нет MCP-клиента)
> **Дата:** 2026-09-26
...
## 6. Фаза 3 — мутации, запись, прочее
...
| 3.5 | `/batch` — список синхронных шагов одним запросом (same-frame; `step`/`shot` отклоняются) | ✅ |
...
| Риск | Митигация |
|---|---|
| Сервер может «нажимать кнопки» — безопасность | по умолчанию `127.0.0.1`; внешний `bind` без токена запрещён; в release off |
...
## 9. Удалённый доступ и деплой

- `ControlConfig` получил `bind` (`control.bind` в `settings.json` или
  `KLEP_CONTROL_BIND`, по умолчанию `127.0.0.1`). При небезопасном адресе без
  токена сервер логирует ошибку и **не стартует**.
- Варианты удалённого управления:
  - прямой bind (`0.0.0.0` или tailscale-IP) + токен;
  - туннели без правок кода: SSH `-L 15703:127.0.0.1:15703` или
    `tailscale serve --bg 15703` (оба стучатся в loopback, поэтому работают и
    при дефолтном bind).
- Проверено на `cachyos-bc-250` (Linux x86_64, AMD RADV GFX1013): проект собран
  на самой машине (`cargo build -p client -p editor_client -p ktrl`, ~9 мин),
  запущен в её Wayland-сессии (`WAYLAND_DISPLAY=wayland-0`, `XDG_RUNTIME_DIR=
  /run/user/1000`) с `KLEP_CONTROL_BIND=0.0.0.0` + токеном; с MacBook по
  Tailscale прогнан `api_smoke.sh` — 25/25 ok; скриншоты реально удалённые
  (primary 1280×720, offscreen RTT 256×256); без токена — `401`.
- **Договорённость (если не сказано иначе):** собранные бинарники кладём в
  `~/Games` на удалённой машине.
